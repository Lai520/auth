  ...
  <div class="home">
    <div class="header" style="padding-right: 20px">
      <div style="flex: 1">
        <h3 style="font-size: 18px">
          {{ trade === 1 ? $t("lever.transaction") : $t("miscro.transaction") }}
        </h3>
      </div>
      <div class="flex alcenter" @click="trade = trade === 2 ? 1 : 2">
        ...
        <a class="subtitle bold">{{
          trade === 2 ? $t("lever.transaction") : $t("miscro.transaction")
        }}</a>
      </div>
    </div>
    <div class="container-my" style="overflow: auto">
      <div class="flex alcenter" style="padding-left: 5px">
        ...
        <div style="margin-left: 20px">
          <mu-button
            @click="$router.push('/kline?from=trade&mode=' + trade)"
            icon
          >
            <svg class="icon" aria-hidden="true">
        ...
        >
          ...
          <mu-list>
            <mu-container v-for="(item, index) in quotationsList" :key="index">
              <mu-list-item
                button
                @click="changeCurrency(item.currency_name, item.currency_id)"
              >
                <mu-list-item-action>
                  <mu-avatar size="20">
                    <img :src="item.logo" />
      ...
      <div class="flex" style="padding: 0 20px">
        ...
        >
          ...
          <div style="overflow: hidden">
            ...
            <div class="flex all">
              <mu-text-field
                :type="priceMode === 'shijia' ? 'text' : 'number'"
                v-model="priceMode === 'shijia' ? bestPrice : price"
                :readonly="priceMode === 'shijia'"
                :color="tradeMode === 'buy' ? 'success' : 'error'"
                help-text=""
                suffix="USDT"
                :label="$t('cuy.price')"
            ...
            <div class="mt5">
              <div
                class="left flex alcenter"
                style="text-align: left; line-height: 20px"
              >
                <div class="subtitle nowrap" style="color: rgba(0, 0, 0, 0.54)">
                  {{ $t("center.available") }}
                </div>
              ...
              >
                ...
                <div class="flex1 talr nowrap currency_title">
                  {{ Number(fee).toFixed(2) }}<i class="currency_title">USDT</i>
                </div>
              </div>
            </div>
            <div class="flex buy-sell-button">
              <mu-button class="mt5" @click="buyCoin('buy')" color="success">
                {{ $t("td.buyin") }}
              </mu-button>
              <mu-button class="mt5" @click="buyCoin('sell')" color="error">
                {{ $t("td.sellout") }}
              </mu-button>
            </div>
          </div>
          ...
          <div>
            <div class="flex all">
              <mu-select
                :color="tradeMode === 'buy' ? 'success' : 'error'"
                :label="$t('miscro.mode')"
                v-model="secondsMoneyMode"
                full-width
              >
                <mu-option
                  v-for="(option, index) in secondModes"
                  :key="option.name"
                  :label="option.name"
                  :value="index"
                ></mu-option>
            ...
            <div class="rateHint">
              {{ $t("rate") + secondModes[secondsMoneyMode].micro_trade_fee }} %
            </div>
            <div class="flex all mb15">
              <mu-select
                :help-text="$t('miscro.rate') + ' ' + tsrate + '%'"
                :color="tradeMode === 'buy' ? 'success' : 'error'"
              ...
              >
                <mu-option
                  v-for="(option, index) in secondTimes"
                  :key="index"
            ...
            <div>
              ...
              >
                ...
                <div class="flex1 talr nowrap currency_title">
                  {{
                    tswallet.micro_balance
                      | numberFixed(tswallet.currency_name === "BTC" ? 6 : 2)
                  }}
        ...
        <div class="flex flex-column" style="min-width: 50%">
          <div class="line green talc" style="line-height: 38px">
            <span style="font-size: 16px">{{ newData | numberFixed(4) }}</span>
            <span class="currency_title"> USDT</span>
          </div>
          <div
            class="flex ft12"
            style="line-height: 30px; text-indent: 12px; color: #666"
          >
          ...
          <div class="flex flex1">
            <ul class="list-item ft12 tc">
              ...
              >
                ...
                <span
                  class="info-shadow selli"
                  :style="{ width: (out[1] / sellMaxValue - 0) * 100 + '%' }"
                ></span>
              </li>
              <mu-divider
                inset
              ...
              >
                ...
                <span
                  class="info-shadow"
                  :style="{ width: (buy[1] / inMaxValue - 0) * 100 + '%' }"
                ></span>
              </li>
            </ul>
          </div>
      ...
      <div
        style="width: 100%; height: 5px; margin: 10px 0"
            ...
            <div style="margin: 10px 0" v-for="item in list_content" :key="item.id">
              <div class="flex alcenter">
                <span class="flex1" style="text-align: left">
                  ...
                  >
                  <i class="subtitle" style="font-style: normal">{{
                    timeOffsetFn(item.transaction_time)
                  }}</i>
                </span>
                <span>
                  <mu-button
              ...
              <div class="flex mt5">
                ...
                <div class="talc" style="width: 33%">
                  ...
                  <div class="subb">{{ item.share }} * {{ item.multiple }}</div>
                </div>
                ...
                <div class="talr" style="width: 33%">
                  ...
                  <div class="subb">
                    <mu-button
                      ...
                      small
                      @click.stop="
                        winPriceY = lossPriceY = 0;
                        openSet = true;
                        setId = item.id;
                        lossPrice = winPrice = Number(item.price).toFixed(2);
                      "
                      color="primary"
                    >
                      {{ $t("lever.setloss") }}
            ...
            >
              <div class="flex alcenter">
                <span class="flex1" style="text-align: left">
                  ...
                  ><i class="subtitle ml3" style="font-style: normal">{{
                    timeOffsetFn(item.complete_time)
                  }}</i>
                </span>
                <span> </span>
              </div>
      ...
      <div class="tradeorder-mask">
        <div class="tradeorder" style="top: 7%">
          <div class="tradeorder-title">
            ...
            <i
              @click="showMicroWind = false"
              class="fr iconfont icon-guanbi mr20"
            ></i>
          </div>
          <div
            v-show="activeOrder.status == 1"
            class="container22"
          ...
          >
            ...
            <div class="controlls" v-if="activeOrder.status == 1">
              <div class="display-remain-time">
                {{
                  parseInt(
                    (activeOrder.endTime - Date.parse(new Date())) / 1000
                  )
                }}
              </div>
              <div class="display-remain-time2">{{ $t("lever.nowPrice") }}</div>
              <div class="display-remain-time3">
                {{ newData | numberFixed }}
              </div>
          ...
          <div
            ...
            v-if="activeOrder.status == 2"
          >
            <p style="color: #a0acc2; margin-top: 15px">
              {{ $t("miscro.jiesuanzhong") }}
            </p>
          </div>

          <div
            style="text-align: center; margin-top: 30px"
            v-if="activeOrder.status == 3"
          >
            <p style="font-size: 30px">
              <span v-if="activeOrder.fact_profits > 0" style="color: green"
                >+{{ activeOrder.fact_profits | numberFixed }} USDT</span
              >
          ...
          <div class="tradeorder-top tradeorder-bili">
            <div class="tradeorder-top-list">
              <span data-localize="zdy.fangxiang">{{
                $t("cuy.direction")
              }}</span>
              <span
                :style="
                  activeOrder.type == 1 ? 'color:#1cad90' : 'color:#cd4e65'
                "
                >{{
            ...
            <div class="tradeorder-top-list">
              <span data-localize="zdy.buynum" style="color: #8d9dbc">{{
                $t("miscro.num")
              }}</span>
              <span>{{ activeOrder.number | numberFixed(2) }}</span>
            </div>
            <div class="tradeorder-top-list">
              <span data-localize="zdy.zxjj" style="color: #8d9dbc">{{
                $t("miscro.buyPrice")
              }}</span>
              <span>{{ activeOrder.open_price | numberFixed }} </span>
            </div>
            <div
              class="tradeorder-top-list"
              v-if="activeOrder.status == 1"
              style="overflow: hidden"
            >
              <span
                style="float: left; color: #8d9dbc"
              ...
              <span>
                <span
                  v-if="
                    activeOrder.type == 1 && newData > activeOrder.open_price
                  "
                  class="green"
                  >+
                  {{
                    (activeOrder.number * (activeOrder.profit_ratio * 0.01))
                      | numberFixed
                  }}
                  <!-- {{computedFee(activeOrder.number)}} -->
                </span>
                <span
                  v-if="
                    activeOrder.type == 1 && newData <= activeOrder.open_price
                  "
                  class="red"
                  >-{{ activeOrder.number | numberFixed }}
                </span>

                <span
                  v-if="
                    activeOrder.type == 2 && newData > activeOrder.open_price
                  "
                  class="red"
                  >-{{ activeOrder.number | numberFixed }}
                </span>
          ...
          <div class="tradeorder-btn">
            <span @click="showMicroWind = false">{{
              $t("legaltrade.back")
            }}</span>
          </div>
        </div>
    ...
    >
      <div v-if="trade === 1">
        ...
        >
          ...
          <div class="flex1 talr nowrap currency_title">
            {{ Number(price).toFixed(2) }}<i class="currency_title">USDT</i>
          </div>
        </div>
        <div
          class="left flex alcenter"
        ...
        >
          ...
          <div class="flex1 talr nowrap currency_title">
            {{ Number(bons).toFixed(2) }}<i class="currency_title">USDT</i>
          </div>
        </div>
        <div
          class="left flex alcenter"
      ...
      <div v-if="trade === 2">
        ...
        >
          ...
          <div class="flex1 talr nowrap currency_title">
            {{ tsnumber
              ...
              secondModes[secondsMoneyMode].name
            }}</i>
          </div>
        </div>
        <div
        ...
        >
          ...
          <div class="flex1 talr nowrap currency_title">
            {{ secondTimes[tstime].seconds }}<i class="currency_title">Sec</i>
          </div>
        </div>
        <div
          class="left flex alcenter"
        ...
        >
          ...
          <div class="flex1 talr nowrap currency_title">
            {{ tsrate }}<i class="currency_title">%</i>
          </div>
        </div>
      </div>
      <mu-button
        ...
        >{{ $t("lay.ceil") }}</mu-button
      >
      <mu-button
        slot="actions"
        v-if="trade === 1"
        flat
        :color="tradeMode === 'buy' ? 'green' : 'red'"
        @click="buyContract"
    ...
    >
      ...
      <mu-button
        ...
        flat
        color="primary"
        @click="openPing = false"
        >{{ $t("legal.ceil") }}</mu-button
      >
      <mu-button
        slot="actions"
        v-if="trade === 1"
    ...
    >
      ...
      <mu-button
        ...
        flat
        color="primary"
        @click="cancel(cancelId)"
        >{{ $t("confirm") }}
      </mu-button>
    </mu-dialog>

    <mu-dialog
      :title="$t('lever.setloss')"
      width="600"
      max-width="80%"
      :esc-press-close="false"
      :overlay-close="false"
    ...
    >
      <div>
        ...
        <mu-text-field
          :help-text="$t('lever.expectLoss') + ' ' + lossPriceY"
          center
          :label="$t('lever.stsPrice')"
          v-model="lossPrice"
        >
          <mu-icon
            @click="add(2, 'btn')"
      ...
      <mu-button slot="actions" flat color="primary" @click="openSet = false"
        >{{ $t("legal.ceil") }}
      </mu-button>
      <mu-button
        slot="actions"
    ...
    >
      <mu-appbar color="primary" :title="$t('vote.detail')">
      ...
      <div style="padding: 24px">
        <mu-list class="badge-list-wrap">
          <mu-list-item>
            <mu-list-item-content>
              <mu-list-item-title
                ><b>{{ showCon.symbol }}</b>
                <mu-badge
                  :color="showCon.type == 1 ? 'green' : 'red'"
                  :content="
                    showCon.type == 1 ? $t('lever.dmore') : $t('lever.dshort')
                  "
                ></mu-badge>
                <mu-badge
                  color="info"
                  :content="showCon.multiple + ' ' + $t('lever.timed')"
                ></mu-badge>
                <mu-badge
                  color="info"
                  :content="showCon.share + ' ' + $t('lever.hands')"
                ></mu-badge>
              </mu-list-item-title>
            </mu-list-item-content>
          ...
          <mu-list-item>
            ...
            <mu-list-item-action>
              <mu-badge
                :content="timeOffsetFn(showCon.transaction_time, 1)"
              ></mu-badge>
            </mu-list-item-action>
          </mu-list-item>
          <mu-list-item>
            <mu-list-item-content>
              <mu-list-item-title>{{ $t("lever.bail") }}</mu-list-item-title>
            </mu-list-item-content>
            <mu-list-item-action>
              <mu-badge :content="showCon.origin_caution_money"></mu-badge>
            </mu-list-item-action>
          </mu-list-item>
          <mu-list-item>
            <mu-list-item-content>
              <mu-list-item-title>{{ $t("lever.charge") }}</mu-list-item-title>
            </mu-list-item-content>
            <mu-list-item-action>
              <mu-badge :content="showCon.trade_fee"></mu-badge>
            </mu-list-item-action>
          </mu-list-item>
          <mu-list-item>
            <mu-list-item-content>
              <mu-list-item-title>{{
                $t(showCon.status == 3 ? "lever.closePrice" : "lever.nowPrice")
              }}</mu-list-item-title>
            </mu-list-item-content>
            <mu-list-item-action>
              <mu-badge
                color="orange"
                :content="showCon.update_price"
              ></mu-badge>
            </mu-list-item-action>
          </mu-list-item>
          <mu-list-item>
            ...
            <mu-list-item-action>
              <mu-badge :content="showCon.target_profit_price"></mu-badge>
            </mu-list-item-action>
          </mu-list-item>
          <mu-list-item>
            <mu-list-item-content>
            ...
            <mu-list-item-action>
              <mu-badge :content="showCon.stop_loss_price"></mu-badge>
            </mu-list-item-action>
          </mu-list-item>
          <mu-list-item>
            <mu-list-item-content>
            ...
            <mu-list-item-action>
              <mu-badge
                :color="showCon.profits > 0 ? 'green' : 'red'"
                :content="showCon.profits"
              ></mu-badge>
            </mu-list-item-action>
          </mu-list-item>
          <mu-list-item>
            <mu-list-item-content>
              <mu-list-item-title>{{ $t("td1.status") }}</mu-list-item-title>
            </mu-list-item-content>
            <mu-list-item-action>
              <mu-badge
                :content="
                  $t(showCon.status == 3 ? 'lever.closed' : 'lever.tdin')
                "
                :color="showCon.status == 3 ? 'primary' : ''"
              ></mu-badge>
            </mu-list-item-action>
          </mu-list-item>
        </mu-list>
...
export default {
  ...
  methods: {
    // 计算手续费
    computedFee(num) {
        let fee = this.secondModes[this.activeType].micro_trade_fee
        return Math.floor((num * (1-fee/100))*10000)/10000
    },
    changePercent(value) {
      this.tshared = parseInt(
        (Number(this.user_lever) * this.tmuti * (value / 100)) / this.shareNum
      );
    },
    ...
    init() {
      ...
      this.subscribeMicro();
      this.options = [
        { label: this.$t("center.shi"), value: "shijia" },
        {
          value: "xianjia",
            ...
            for (let i = 0; i < res.data.message.length; i++) {
              for (let j = 0; j < res.data.message[i].quotation.length; j++) {
                ...
                if (
                  ...
                  that.shareNum =
                    res.data.message[i].quotation[j].lever_share_num *
                    res.data.message[i].quotation[j].now_price;
                }
              }
            }
            that.subScribeDepth();
            ...
            that.sockets.subscribe("daymarket", (msg) => {
              if (msg.type == "daymarket") {
                for (let i = 0; i < that.quotationsList.length; i++) {
                  if (
                    that.quotationsList[i].legal_id === msg.legal_id &&
                    that.quotationsList[i].currency_id === msg.currency_id
                  ) {
    ...
    buySecond() {
      ...
      this.$http({
        ...
        if (res.data.type == "ok") {
          ...
          // that.miscroOrder.created_at = that.miscroOrder.created_at.replaceAll('-','/')
          that.miscroOrder.endTime =
            new Date(that.miscroOrder.created_at).getTime() +
            that.miscroOrder.seconds * 1000;
          // console.log(that.miscroOrder);

          that.$toast.success(that.$t("miscro.success"));
          if (that.appSource() !== "ios") {
    ...
    showWind1(order) {
      ...
      let stamp =
        Date.parse(new Date()) -
        Date.parse(new Date(that.activeOrder.created_at));
      ...
      console.log(
        "剩余时间",
        parseInt(
          (Date.parse(new Date(that.activeOrder.handled_at)) -
            Date.parse(new Date(that.activeOrder.created_at))) /
            1000
        )
      );
      ...
      this.timer = setInterval(function () {
        if (
          parseInt(
            (that.activeOrder.endTime - Date.parse(new Date())) / 1000
          ) <= 0
        ) {
            ...
            that
              ...
              .then((res) => {
                if (res.data.type === "ok") {
                  ...
                  that.activeOrder =
                    order.length > 0 ? order[0] : that.activeOrder;
                }
              });
          }, 8000);
        } else {
      ...
      if (my.type === 1) {
        ...
        if (type == 1) {
          ...
          if (froms == "btn") {
            ...
            console.log(that.winPrice);
          }
          console.log("买跌止盈", froms);
          console.log(my.price - that.winPrice * my.share);
          that.winPriceY = (
      ...
      if (my.type === 1) {
        ...
        if (type == 1) {
          ...
          if (that.lossPrice > 0) {
            ...
            that.lossPriceY = (
              (parseFloat(that.lossPrice - my.price) - 0) *
              (my.share - 0)
            ).toFixed(2);
          } else {
            that.$toast.warning(that.$t("lever.thanzone"));
    ...
    // 时间时区转换
    timeOffsetFn(time, noSub) {
      if (noSub) return timeOffset(time);
