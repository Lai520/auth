  ...
  <div style="height: 100vh; overflow: auto">
    <mu-container>
      ...
      <mu-appbar style="width: 100%" color="#fff" text-color="#000">
        ...
        <mu-button icon flat slot="right"> </mu-button>
      </mu-appbar>
      ...
      <div v-if="charge == 'USDT' && goBank == 0" class="flex mb5">
        <div class="flex1"></div>
        <div
          v-if="showERC"
          @click="active = 0"
      ...
      <div style="padding: 0 20px">
        <mu-text-field
          ...
          :label="$t('dep.ti_num')"
          :suffix="charge"
          v-model="wallets.change_balance"
          readonly=""
        ></mu-text-field>
        <mu-text-field
          v-if="!goBank"
          style="width: 100%; margin-bottom: 20px !important"
          :help-text="$t('wAddress.nospace')"
          :label="$t('wAddress.enterAddress')"
          v-model="address"
        ></mu-text-field>
        <mu-text-field
          style="width: 100%; margin-bottom: 20px !important"
        ...
        >
          <span
            ...
            "
          >
            <mu-button :disabled="sended" flat color="primary">{{
              $t("send")
            }}</mu-button>
          </span>
        </mu-text-field>
        <div
          ...
          style="line-height: 30px; margin-left: 0"
        >
          {{ $t("account.havenum")
          }}<b style="color: #000">{{
            number - wallets.rate >= 0 ? number - wallets.rate : 0
          }}</b
          >{{ charge }}
        </div>
        <mu-button
        ...
        <div
          v-if="!info && showCustomer"
          style="
            color: rgb(33 143 221);
            text-align: left;
            width: 100%;
            line-height: 2;
            margin-top: 10px;
        ...
        >
          {{ $t("bank.binded") }}<br />
          {{ info.bank_name }} :<b style="color: #331f00"
            >********{{
              info.bank_account.substring(
                info.bank_account.length - 4,
                info.bank_account.length
              )
            }}</b
          >
        </div> -->
        ...
        <mu-button
          ...
          style="
            margin-top: 10px;
            background: linear-gradient(to right, #00cf66, #035231);
          "
          >{{ $t("bank.withdraw") }}
        </mu-button>
        <p class="tall hide">
    ...
    >
      ...
      <div style="padding: 0px 0">
        ...
        <div
          class="go-w"
          @click="withdrawBank"
          v-if="getConfigInfo('bank_cash') == 1"
        >
          {{ $t("bank.withdraw") }}
        </div>
      </div>
