  ...
  >
    <div class="wrap">
      <div>
        <!--轮播图开始-->
        <swiper
          v-if="getConfigInfo('banner_show') == 1"
        ...
        >
          ...
          >
            <img
              :src="slide.pic"
              style="width: 100%; border-radius: 5px; height: auto"
            />
          </swiper-slide>
        ...
        >
          ...
          <div
            ...
            style="height: 40px; flex: 1; overflow: hidden"
          >
            <div class="textBox">
              <transition name="slide">
                <p
                  class="text flex"
                  style="padding: 0 10px; line-height: 42px"
                  :key="text.id"
                >
                  <span class="noticeMsg">{{ text.val.title }}</span>
                  <span>{{ text.val.create_time }}</span>
                </p>
              </transition>
            </div>
          </div>
          ...
          <template v-for="(item1, index) in quotationsListPrice">
            <div style="flex: 1" :key="index" v-if="index < 3">
              <span style="font-size: 14px; font-weight: bold"
                >{{ item1.currency_name }}/{{ item1.legal_name }}</span
              >
              <br />
              <span
                style="font-weight: bold; font-size: 16px"
                :class="item1.change > 0 ? 'green' : 'red'"
                >{{ item1.now_price }}</span
              >
              <br />
              <span :class="item1.change > 0 ? 'green' : 'red'"
                >{{ item1.change }}%</span
              >
              <br />
              <span
                class="bold"
                style="font-size: 10px; color: rgb(170, 170, 170)"
              >
                $
                {{
                  ((item1.volume * item1.now_price) / 1000000) | numberFixed(2)
                }}
                M
              </span>
            </div>
      ...
      ></div>
      <div
        class="inner-menu flex"
        :style="getDayColor('border-color')"
          ...
          <mu-ripple>
            <div class="pt5">
              ...
              <template v-if="item.link == 'pledge'">
                <div
                  v-if="!getWalletAddress"
                  class="bold"
                  :style="index == 3 ? 'color:#3586fd' : ''"
                >
                  {{ item.name }}
                </div>
                <div
      ...
      <div style="width: 100%; min-height: 500px">
        <mu-container>
          <!-- <mu-tabs
            :value.sync="active1"
            :inverse="mode == 'day'"
            :color="mode == 'day' ? 'black' : '#303030'"
            :indicator-color="mode == 'day' ? 'white' : 'white'"
            left
          >
            <mu-tab style="font-weight: bold"
              ><span
                style="padding: 0 5px; font-size: 16px; font-weight: bold"
                >{{ $t("market.quotation") }}</span
              >
            </mu-tab>
            <mu-tab style="font-weight: bold"
              ><span
          ...
          <div
            class="bold tall ft16"
            style="padding-left: 20px; line-height: 50px"
          >
            {{ $t("market.quotation") }}
          </div>
          <div>
            <div
              class="flex alcenter bold"
              style="
            ...
            <template v-for="(item, i) in showQuotationList">
              <div
                class="flex alcenter"
                @click="gotoKline(item.currency_id, item.currency_name)"
                style="
                  border-top: 1px solid #fafafa;
                  padding-top: 10px;
                  padding: 10px 20px;
              ...
              >
                <div class="flex flex-column" style="flex: 1; text-align: left">
                  ...
                  <span class="fColor1 ml10" style="color: rgb(170, 170, 170)"
                    >USDT</span
                  >
                </div>
                <div
                  class="flex flex-column flex1"
                ...
                >
                  ...
                  >
                    $
                    {{
                      ((item.volume * item.now_price) / 1000000)
                        | numberFixed(2)
                    }}
                    M
                  </span>
      ...
      <mu-alert
        v-if="showDownloadBtn && false"
        @delete="showDownloadBtn = false"
        color="info"
        delete
      ...
      >
        ...
        />
        <a class="downloadApp" href="/mobile/download.html">App Download</a>
      </mu-alert>
    </div>
...
export default {
  ...
  watch: {
    active1(val) {
      this.showQuotationList =
        val === 0
          ? this.quotationsListChange
          : val === 1
          ? this.quotationsListPrice
          : this.quotationsListVol;
    },
  },
  mounted() {
    instance = this;
            ...
            for (let i = res.data.message.list.length - 1; i >= 0; i--) {
              ...
              pics.push({
                ...
                create_time: item.create_time
                  ? this.timeOffsetFn(item.create_time).substring(5, 10)
                  : "",
              });
            }
            pics = pics.sort((x, y) => {
