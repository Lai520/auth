    ...
    <mu-container>
      ...
      <mu-form ref="form" :model="validateForm" v-if="stepOne" class="mu-demo-form">
        ...
        <div class="flex" style="height: 50px;">
          ...
          <span v-if="mode==1 && showMobileRegister" @click="mode=2" class="flex alcenter">{{$t('bdemail.emailbox')}}</span>
          <span v-if="mode==2 && showMobileRegister" @click="mode=1" class="flex alcenter">{{$t('bdphone.phone')}}</span>
        </div>
      </mu-form>
...
export default {
  ...
  methods: {
    ...
    sendCode() {
      ...
      if(this.mode==2)
      {
        data.area_code = this.validateForm.area_code;
      }
